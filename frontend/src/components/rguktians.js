...
class rguktians extends Component {
    ...
    render() {
        ...
        const cards = studentsearch.length === 0 ? [] : (
            studentsearch.map((item) => {
                return (
                <Grid item xs={12} md={4} style={{ padding: '1%' }} key={item.url}>
                    <Card className='dark-mode' style={{ alignItems: 'center', height: '100%', paddingTop: '5%' }}>
                        <CardImg style={{ height: '125px', width: '125px', borderRadius: '50%' }} top src={item.profile} alt="Card image cap" />
                        <CardBody>
        ...
        const pugs = studentsearch.length === 0 ? [] : (
            studentsearch.map((item) => {
            var temp = []
                    ...
                    <Card className='dark-mode' style={{ alignItems: 'center' }}>
                        ...
                        <CardBody>
                            ...
                            <Grid style={{ alignItems: 'center' }} justify='center' container direction="row">
                                ...
                                <Grid item xs={2}>
                                    <Button style={{ background: 'transparent', color: 'inherit', borderColor: '#e3e3e3' }} onClick={(e) => { e.preventDefault(); window.open(item.email, "_blank").focus(); }}>&lt;&gt;</Button>
                                </Grid>
                            </Grid>
                        </CardBody>
                    </Card>
        ...
        return (
            <>
                <MapContainer style={{ height: '75vh' }} center={[20.5937, 78.9629]} zoom={4.5} scrollWheelZoom={false}>
                    <TileLayer
